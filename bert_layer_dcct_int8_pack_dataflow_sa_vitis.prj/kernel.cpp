...
void Attention_layer(
  ...
  gemm_systolic_array_attn(inp, W, acc_outp, head_id);
...
void Context_layer(
  ...
  gemm_systolic_array_cont(inp, W, acc_outp, head_id);
  ...
  #pragma HLS UNROLL
    ...
    #pragma HLS pipeline II=1
      ...
      int8_t outp_i8 = outp1_f;
      outp[i9].write(outp_i8);        // L81
    }
  }
}
...
void Self_attention(
  ...
  int8_t C[head_num][inp_num][head_len];
  #pragma HLS array_partition variable=C complete dim=1
  #pragma HLS array_partition variable=C complete dim=2

  l_S_h_0_h: for (int h = 0; h < head_num; h++) {     // L150
    hls::stream<float> attn_outp[inp_num];
    #pragma HLS STREAM variable=attn_outp depth=4
    hls::stream<int8_t> sfm_outp[inp_num];
    #pragma HLS STREAM variable=sfm_outp depth=4
    hls::stream<int8_t> cont_outp[inp_num];
    #pragma HLS STREAM variable=cont_outp depth=4

    Attention_layer(inp, K, s_attn, attn_outp, h);      // L164
    Softmax_layer(attn_outp, s_sfm, sfm_outp);    // L165
    Context_layer(sfm_outp, V, s_cont, cont_outp, h);       // L166

    l_write_i: for (int i = 0; i < inp_num; i++) {
    #pragma HLS UNROLL
      l_j: for (int j = 0; j < head_len; j++) {
      #pragma HLS pipeline II=1
        C[h][i][j] = cont_outp[i].read();
      }
    }
  }
  ...
  #pragma HLS UNROLL
    l_j: for (int j = 0; j < inp_len; j++) {
    #pragma HLS pipeline II=1
      outp[i].write(C[j / head_len][i][j % head_len]);
    }
  }
}
...
void Bert_layer(
  ...
  l_S_read_inp_i: for (int i = 0; i < inp_num; i++) {	//
    l_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
  ...
  l_S_result29_i: for (int i = 0; i < inp_num; i++) {	//
    l_j: for (int j = 0; j < inp_len; j++) {	//
