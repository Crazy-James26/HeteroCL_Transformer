...
def top():
    ...
    def Linear_layer_qkv(inp: q_type[inp_num, inp_len], W: q_type[inp_len, inp_len], B: q_type[inp_len]) -> q_type[inp_num, inp_len]:
        outp: q_type[inp_num, inp_len] = 0
    ...
    def Attention_layer(Q_h: q_type[inp_num, head_len], K_h: q_type[inp_num, head_len]) -> float32[inp_num, inp_num]:
        outp: q_type[inp_num, inp_num] = 0
        outp_f: float32[inp_num, inp_num] = 0
    ...
    def Context_layer(Attn: q_type[inp_num, inp_num], V_h: q_type[inp_num, head_len]) -> q_type[inp_num, head_len]:
        outp: q_type[inp_num, head_len] = 0
        for i, j in allo.grid(inp_num, head_len, name="gemm"):
                for k in allo.reduction(inp_num):
                        outp[i, j] += Attn[i, k] * V_h[k, j]
        return outp

    def Self_attention(Q: q_type[inp_num, inp_len],
    ...
    def Linear_layer_ds0(inp: q_type[inp_num, inp_len], W: q_type[inp_len, inp_len], B: q_type[inp_len]) -> q_type[inp_num, inp_len]:
        outp: q_type[inp_num, inp_len] = 0
        for i, j in allo.grid(inp_num, inp_len, name="gemm"):
            for k in allo.reduction(inp_len):
                outp[i, j] += inp[i, k] * W[j, k]
    ...
    def Res_layer(inp1: q_type[inp_num, inp_len], inp2: q_type[inp_num, inp_len]) -> float32[inp_num, inp_len]:
        outp: float32[inp_num, inp_len]
        for i, j in allo.grid(inp_num, inp_len):
            outp[i, j] = inp1[i, j] + inp2[i, j]
        return outp

    def Layer_norm(inp: float32[inp_num, inp_len], gamma: float32[inp_len], beta: float32[inp_len]) -> q_type[inp_num, inp_len]:
    ...
    def Linear_layer_ds1(inp: q_type[inp_num, inp_len], W: q_type[gelu_len, inp_len], B: q_type[gelu_len]) -> float32[inp_num, gelu_len]:
        outp: q_type[inp_num, gelu_len] = 0
        outp_f: float32[inp_num, gelu_len] = 0
        for i, j in allo.grid(inp_num, gelu_len, name="gemm"):
            for k in allo.reduction(inp_len):
                outp[i, j] += inp[i, k] * W[j, k]
        for i, j in allo.grid(inp_num, gelu_len, name="bias"):
            outp_f[i, j] = outp[i, j] + B[j]
        return outp_f
    ...
    def Linear_layer_ds2(inp: q_type[inp_num, gelu_len], W: q_type[inp_len, gelu_len], B: q_type[inp_len]) -> q_type[inp_num, inp_len]:
        outp: q_type[inp_num, inp_len] = 0
        for i, j in allo.grid(inp_num, inp_len, name="gemm"):
    ...
    def Bert_layer(inp: q_type[inp_num, inp_len], 
                    Wq: q_type[inp_len, inp_len], Bq: q_type[inp_len], 
                    Wk: q_type[inp_len, inp_len], Bk: q_type[inp_len], 
                    Wv: q_type[inp_len, inp_len], Bv: q_type[inp_len],
                    output_dense_w: q_type[inp_len, inp_len], output_dense_b: q_type[inp_len],
                    ffn_w1: q_type[gelu_len, inp_len], ffn_b1: q_type[gelu_len], 
                    ffn_w2: q_type[inp_len, gelu_len], ffn_b2: q_type[inp_len], 
                    gamma1: float32[inp_len], beta1: float32[inp_len], 
                    gamma2: float32[inp_len], beta2: float32[inp_len]) -> q_type[inp_num, inp_len]:
        # 1. Bert Attention
        # 1.0 project Q, K, V
