...
void input_loader_kv(
  ...
  #pragma HLS pipeline II=1
    l_load_i: for (int i = 0; i < inp_num; i++) {
      int8_t data_temp = inp[0][j].data[i] * s[i];
...
void K_writer(
  ...
  #pragma HLS pipeline II=1
    data_pack = inp.read();
    K[j / head_len].k_data[0][j % head_len] = data_pack;
  }
}
...
void V_writer(
  ...
  #pragma HLS pipeline II=1
    ...
    l_write_i: for (int i = 0; i < inp_num; i++) {
      V[j / head_len].v_data[i][(j % head_len) / inp_num].data[j % inp_num] = data_pack.data[i];
    }
  }
}
...
void input_loader_q_res0(
  ...
  l_load_j: for (int j = 0; j < inp_len; j++) {
  #pragma HLS pipeline II=1
    inp_res.write(inp[0][j]);
    l_load_i: for (int i = 0; i < inp_num; i++) {
      int8_t data_temp = inp[0][j].data[i] * s[i];   // L9
...
void Attention_layer(
  ...
  gemm_systolic_array_attn(inp_buf, K[head_id].k_data, s, outp);
...
void Context_layer(
  ...
  gemm_systolic_array_cont(inp_buf, V[head_id].v_data, s, outp);
...
void Self_attention(
  ...
  #pragma HLS DATAFLOW
    Attention_layer(inp, K, s_attn, attn_outp, h);
    Softmax_layer(attn_outp, s_sfm, sfm_outp);
    Context_layer(sfm_outp, V, s_cont, outp, h);
  }
}
  ...
  #pragma HLS pipeline II=1
    ...
    l_i20: for (int i20 = 0; i20 < inp_num; i20++) {   // L322
      ...
      if (inp_data < 3)
        ...
        outp_data = inp_data;
      outp_data_pack.data[i20] = outp_data * s[i20];    // L343
    }
    outp.write(outp_data_pack);
  }
}
...
void Bert_layer(
  ...
  l_read_buf1_i: for (int i = 0; i < pack2_inp_len; i++) {	//
    l_read_buf1_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
      buf1[i][j] = buf1_addr[i * inp_len + j];	//
    }
  }

  l_read_buf3_i: for (int i = 0; i < pack2_inp_len; i++) {	//
    l_read_buf3_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
      buf1[i][j] = buf3_addr[i * inp_len + j];	//
    }
  }

  l_read_buf5_i: for (int i = 0; i < pack2_inp_len; i++) {	//
    l_read_buf5_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
      buf5[i][j] = buf5_addr[i * inp_len + j];	//
    }
  }

  l_read_buf7_i: for (int i = 0; i < pack2_inp_len; i++) {	//
    l_read_buf7_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
      buf7[i][j] = buf7_addr[i * inp_len + j];	//
    }
  }

  l_read_buf9_i: for (int i = 0; i < pack2_gelu_len; i++) {	//
    l_read_buf9_j: for (int j = 0; j < inp_len; j++) {	//
    #pragma HLS pipeline II=1
      buf9[i][j] = buf9_addr[i * inp_len + j];	//
    }
  }

  l_read_buf11_i: for (int i = 0; i < pack2_inp_len; i++) {	//
    l_read_buf11_j: for (int j = 0; j < gelu_len; j++) {	//
    #pragma HLS pipeline II=1
      buf11[i][j] = buf11_addr[i * gelu_len + j];	//
    }
  }

  Bert_layer_dataflow_region_1(inp, buf1, buf3, K, V);
  Bert_layer_dataflow_region_2(inp, buf5, buf7, buf9, buf11, K, V, outp);
