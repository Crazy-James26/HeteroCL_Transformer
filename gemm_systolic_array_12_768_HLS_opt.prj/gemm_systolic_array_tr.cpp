...
void gemm_systolic_array_tr(d_type A[M][K], d_type B[N][K], d_type C[M][N]){

	#pragma HLS ARRAY_PARTITION variable = A cyclic factor = block_M dim = 1
    #pragma HLS ARRAY_PARTITION variable = B cyclic factor = block_N dim = 1
	...
	#pragma HLS ARRAY_PARTITION variable = block_C
	...
	for(int ii = 0; ii < M/block_M; ii++){
		...
		#pragma HLS DATAFLOW
			...
			for(int k = 0; k < K; k++){
				for(int j = 0; j < block_N; j++){
				#pragma HLS UNROLL
					block_B_loader[j].write(B[jj * block_N + j][k]);
				}
			}
