...
void K_writer(
  ...
  #pragma HLS pipeline II=1
    data_pack = inp.read();
    K[j / head_len].k_data[pack_seq_id][j % head_len] = data_pack;
  }
}
...
void V_writer(
  ...
  #pragma HLS pipeline II=1
    ...
    l_write_i: for (int i = 0; i < inp_num; i++) {
      V[j / head_len].v_data[pack_seq_offset + i][(j % head_len) / inp_num].data[j % inp_num] = data_pack.data[i];
    }
  }
}
...
void Attention_layer(
  ...
  gemm_systolic_array_attn(inp_buf, K[head_id].k_data, s, outp, pack_seq_offset);
...
void Context_layer(
  ...
  gemm_systolic_array_cont(inp_buf, V[head_id].v_data, s, outp, pack_seq_offset);
