...
void Linear_layer_k(
  ...
  io_pack_int inp_data_pack;
  l_j1: for (int j1 = 0; j1 < inp_len; j1++) {    // L19
    #pragma HLS pipeline II=1
    inp_data_pack = inp.read();
    l_i1: for (int i1 = 0; i1 < inp_num; i1++) {  // L18
      inp_buf[i1][j1] = inp_data_pack.data[i1];
    }
  }

  gemm_systolic_array_qkv(inp_buf, W, B, s, outp);
...
void K_writer(
  ...
  #pragma HLS pipeline II=1
    ...
    l_write_i: for (int i = 0; i < inp_num; i++) {
      K[j / head_len].data[i][j % head_len] = data_pack.data[i];
    }
  }
}
...
void V_writer(
  ...
  #pragma HLS pipeline II=1
    data_pack = inp.read();
    l_write_i: for (int i = 0; i < inp_num; i++) {
      V[j / head_len].data[i][j % head_len] = data_pack.data[i];
    }
  }
}
...
void input_loader_q_res0(
  ...
  io_pack_float inp_data_pack_res;
  io_pack_int inp_data_pack_q;
  l_load_j: for (int j = 0; j < inp_len; j++) {
  #pragma HLS pipeline II=1
    l_load_i: for (int i = 0; i < inp_num; i++) {
      float data_temp_f = inp[i][j];
      inp_data_pack_res.data[i] = data_temp_f;
      int8_t data_temp_i8 = data_temp_f * s[i];   // L9
      inp_data_pack_q.data[i] = data_temp_i8;
    }
    inp_res.write(inp_data_pack_res);
...
void Attention_layer(
  ...
  gemm_systolic_array_attn(inp_buf, W, s, outp);
...
void Softmax_layer(
  ...
  l_exp_sum_j6: for (int j6 = 0; j6 < inp_num; j6++) {     // L92
  #pragma HLS pipeline II=1
    inp_data_pack = inp.read();
    l_exp_sum_i6: for (int i6 = 0; i6 < inp_num; i6++) {       // L91
      float v69 = inp_data_pack.data[i6];
...
void Context_layer(
  ...
  #pragma HLS SHARED variable=W
  #pragma HLS array_partition variable=W cyclic dim=2 factor=16

  int8_t inp_buf[inp_num][inp_num];
  #pragma HLS array_partition variable=inp_buf complete dim=1
  
  io_pack_int inp_data_pack;
  l_j1: for (int j1 = 0; j1 < inp_num; j1++) {    // L19
    #pragma HLS pipeline II=1
    inp_data_pack = inp.read();
    l_i1: for (int i1 = 0; i1 < inp_num; i1++) {  // L18
      inp_buf[i1][j1] = inp_data_pack.data[i1];
  ...
  gemm_systolic_array_cont(inp_buf, W, s, outp);
...
void Self_attention(
  ...
  hls::stream<io_pack_int> sfm_outp;
  #pragma HLS STREAM variable=sfm_outp depth=4
  
  l_multi_head: for (int h = 0; h < head_num; h++){
  #pragma HLS DATAFLOW
    Attention_layer(inp, K[h].data, s_attn, attn_outp);
    Softmax_layer(attn_outp, s_sfm, sfm_outp);
    Context_layer(sfm_outp, V[h].data, s_cont, outp);
  }
}
...
void Linear_layer_ds0(
  ...
  gemm_systolic_array_ds0(inp_buf, W, B, s, outp);
...
void Layer_norm0(
  ...
  #pragma HLS pipeline II=1
    l_mean_var_i16: for (int i16 = 0; i16 < inp_num; i16++) {
      float temp = (buf[i16][j16] - mean[i16]) / sqrt(var[i16] + 0.000010);
...
void input_loader_ds1_res1(
  ...
  #pragma HLS pipeline II=1
    ...
    l_loader_i: for (int i = 0; i < inp_num; i++) {        // L4
      int8_t data_temp = inp_data_pack.data[i] * s[i];   // L9
...
void Linear_layer_ds1(
  ...
  gemm_systolic_array_ds1(inp_buf, W, B, s, outp);
...
void Linear_layer_ds2(
  ...
  gemm_systolic_array_ds2(inp_buf, W, B, s, outp);
...
void output_writer(
  ...
  #pragma HLS pipeline II=1
    outp_data_pack = inp.read();
    l_scale_outp_i: for (int i = 0; i < inp_num; i++) {
      outp[i][j] = outp_data_pack.data[i];
    }
  }
}
...
#pragma HLS DATAFLOW
  ...
  input_loader_q_res0(inp, buf17, inp_res0, inp_q);
  hls::stream<io_pack_int> outp_q;
  #pragma HLS STREAM variable=outp_q depth=4
  Linear_layer_q(inp_q, buf5, buf6, buf20, outp_q);	// L460
  hls::stream<io_pack_int> outp_sfa;
  ...
  hls::stream<io_pack_int> inp_ds1;
  ...
  hls::stream<io_pack_int> outp_gelu;
...
void Bert_layer(
  ...
  #pragma HLS array_partition variable=buf1 cyclic dim=1 factor=inp_parallel

  #pragma HLS array_partition variable=buf3 cyclic dim=1 factor=inp_parallel

  #pragma HLS array_partition variable=buf5 cyclic dim=1 factor=inp_parallel

  #pragma HLS array_partition variable=buf7 cyclic dim=1 factor=inp_parallel

  #pragma HLS array_partition variable=buf9 cyclic dim=1 factor=inp_parallel

  #pragma HLS array_partition variable=buf11 cyclic dim=1 factor=inp_parallel

  MH_K K[head_num];
  #pragma HLS SHARED variable=K
  #pragma HLS array_partition variable=K complete dim=1

  MH_V V[head_num];
  #pragma HLS SHARED variable=V
