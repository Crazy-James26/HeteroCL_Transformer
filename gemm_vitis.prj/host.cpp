...
int main(int argc, char** argv) {
    ...
    // CL_MEM_USE_HOST_PTR
    size_t size_bytes_in0 = sizeof(int) * 1024;
    std::vector<int, aligned_allocator<int> > source_in0(1024);
    size_t size_bytes_in1 = sizeof(int) * 1024;
    std::vector<int, aligned_allocator<int> > source_in1(1024);
    size_t size_bytes_out0 = sizeof(int) * 1024;
    std::vector<int, aligned_allocator<int> > source_out0(1024);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_gemm = cl::Kernel(program, "gemm", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_gemm.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_gemm.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_gemm.setArg(2, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_gemm, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "gemm: "
