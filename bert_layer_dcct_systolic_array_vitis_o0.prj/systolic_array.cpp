...
void PE(hls::stream<ap_fixed<24, 8>>& A_in, hls::stream<ap_fixed<24, 8>>& A_out,
	...
	#pragma HLS PIPELINE II=1
		ap_fixed<24, 8> a = A_in.read();
		ap_fixed<24, 8> b = B_in.read();
		C_out += a * b;
		A_out.write(a);
		B_out.write(b);
	}
...
void systolic_array_k_768(hls::stream<ap_fixed<24, 8>> A_loader[block_size1], 
					hls::stream<ap_fixed<24, 8>> B_loader[block_size1], 
					hls::stream<ap_fixed<24, 8>> C_drainer[block_size1]) {
    hls::stream<ap_fixed<24, 8>> A_fifo[block_size1][block_size1 + 1];
    hls::stream<ap_fixed<24, 8>> B_fifo[block_size1][block_size1 + 1];

    #pragma HLS STREAM variable=A_fifo depth=2
    #pragma HLS STREAM variable=B_fifo depth=2

    ap_fixed<24, 8> C[block_size1][block_size1];
...
void systolic_array_k_3072(hls::stream<ap_fixed<24, 8>> A_loader[block_size1], 
					hls::stream<ap_fixed<24, 8>> B_loader[block_size1], 
					hls::stream<ap_fixed<24, 8>> C_drainer[block_size1]) {
    hls::stream<ap_fixed<24, 8>> A_fifo[block_size1][block_size1 + 1];
    hls::stream<ap_fixed<24, 8>> B_fifo[block_size1][block_size1 + 1];
...
void systolic_array_k_64(hls::stream<ap_fixed<24, 8>> A_loader[block_size2], 
					hls::stream<ap_fixed<24, 8>> B_loader[block_size2], 
					hls::stream<ap_fixed<24, 8>> C_drainer[block_size2]) {
    hls::stream<ap_fixed<24, 8>> A_fifo[block_size2][block_size2 + 1];
    hls::stream<ap_fixed<24, 8>> B_fifo[block_size2][block_size2 + 1];

    #pragma HLS STREAM variable=A_fifo depth=2
    #pragma HLS STREAM variable=B_fifo depth=2

    ap_fixed<24, 8> C[block_size2][block_size2];
...
void systolic_array_k_12(hls::stream<ap_fixed<24, 8>> A_loader[block_size2], 
					hls::stream<ap_fixed<24, 8>> B_loader[block_size2], 
					hls::stream<ap_fixed<24, 8>> C_drainer[block_size2]) {
    hls::stream<ap_fixed<24, 8>> A_fifo[block_size2][block_size2 + 1];
    hls::stream<ap_fixed<24, 8>> B_fifo[block_size2][block_size2 + 1];
