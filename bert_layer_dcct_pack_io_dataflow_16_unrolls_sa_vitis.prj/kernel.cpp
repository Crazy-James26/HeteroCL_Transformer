...
void Self_attention(
  ...
  hls::stream<io_pack_int> attn_inp[head_num];
  #pragma HLS STREAM variable=attn_inp depth=4
  hls::stream<io_pack_float> attn_outp[head_num];
  #pragma HLS STREAM variable=attn_outp depth=4
  hls::stream<io_pack_int> sfm_outp[head_num];
  #pragma HLS STREAM variable=sfm_outp depth=4
  hls::stream<io_pack_int> cont_outp[head_num];
  #pragma HLS STREAM variable=cont_outp depth=4
  
  #pragma HLS DATAFLOW

  l_inp_split: for (int j = 0; j < inp_len; j++) {
    #pragma HLS pipeline II=1
    attn_inp[j / head_len].write(inp.read());
  }

  l_multi_head: for (int h = 0; h < head_num; h++){
  #pragma HLS UNROLL
    Attention_layer(attn_inp[h], K[h].data, s_attn, attn_outp[h]);
    Softmax_layer(attn_outp[h], s_sfm, sfm_outp[h]);
    Context_layer(sfm_outp[h], V[h].data, s_cont, cont_outp[h]);
  }

  l_outp_merge: for (int j = 0; j < inp_len; j++) {
    #pragma HLS pipeline II=1
    outp.write(cont_outp[j / head_len].read());
  }
}
